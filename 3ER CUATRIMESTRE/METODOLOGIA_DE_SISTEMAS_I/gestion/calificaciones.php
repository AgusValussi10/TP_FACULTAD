...
<head>
  ...
  <title>Registrar Calificaciones – Educar para Transformar</title>
...
<div class="container">
  ...
  <div class="card">
    <div class="card-header"><span class="icon">✏️</span><h2>Calificaciones (0 a 10)</h2></div>
    <div class="card-body">
      <div class="form-row">
        <label>Materia
        ...
        <label>Evaluación
          <input type="text" id="sel-evaluacion" placeholder="Ej: TP N°3" maxlength="150">
        </label>
...
<script>
  ...
  const selEvaluacion = document.getElementById('sel-evaluacion');
  ...
  btnCargar.addEventListener('click', async () => {
    ...
    const evaluacion = selEvaluacion.value.trim();
    const fecha = selFecha.value;
    if (!materia_id || !evaluacion || !fecha) { setMsg('Completá materia, evaluación y fecha.', '#DC2626'); return; }
    ...
    try {
      const url = `calificaciones_listar.php?materia_id=${encodeURIComponent(materia_id)}&evaluacion=${encodeURIComponent(evaluacion)}`;
